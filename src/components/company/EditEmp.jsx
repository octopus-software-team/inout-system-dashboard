...
const EditEmp = () => {
  const { id } = useParams(); // استخدم useParams مباشرة هنا
  ...
  const [imagePreview, setImagePreview] = useState(null); // إضافة حالة للمعاينة
  ...
  useEffect(() => {
    ...
    const fetchEmployeeData = async () => {
      try {
        const response = await fetch(
          `https://inout-api.octopusteam.net/api/front/getEmployee/${id}`,
          {
            method: "GET",
            headers: {
              Authorization: `Bearer ${token}`,
    ...
    fetchBranches();
    fetchSpecialties();
    fetchEmployeeData(); // استخدم id هنا مباشرة بدلاً من match.params.id
  }, [token, id]); // ملاحظة أنه عند التغيير في id سيحدث التحديث
  ...
  const handleChange = (e) => {
    ...
    if (type === "file") {
      ...
      setImagePreview(URL.createObjectURL(files[0])); // تعيين رابط الصورة المؤقت للمعاينة
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://inout-api.octopusteam.net/api/front/updateEmployee/${formData.id}`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (response.ok && result.status === 200) {
        setMessage("Engineer updated successfully!");
        setTimeout(() => {
          window.location.reload();
        }, 2000);
      } else {
        setMessage("Error updating engineer, please try again.");
      }
    } catch (error) {
      console.error(error);
